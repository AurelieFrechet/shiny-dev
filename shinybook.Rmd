...
Par défaut lorsqu'on exécute la commande suivante, c'est la version de R correspondant à la version du système d'exploitation Ubuntu qui est installée. Or cette version de R date de deux ans ou plus. 

```{}
sudo apt-get install r-base
```

Pour installer la dernière version de R il faut tout d'abord ajouter le répertoire CRAN au système d'exploitation Ubuntu. Cela permet d'installer la version la plus récente de R car autrement une version antérieure de R (`3.4`) va être installée.  
...
Le serveur écoute au port 3838. Il est possible de changer ce port.  

Le dossier que le serveur de Shiny sert est situé au `/srv/shiny-server`.  
...
Je copie ce fichier vers le dossier qui est servi par le serveur Shiny. Toute opération dans ce dossier requiert des privilèges administrateur.  
...
Bien qu'il soit intéressant de servir des pages HTML directement à partir du shiny vous vous en doutez que ce n'est pas à cela que Shiny server sert.  
